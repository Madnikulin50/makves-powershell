param (
    [string[]]$paths = "",
    [bool]$localdisks = $false,
    [string[]]$filters = @("*.xlsx", "*.xls", "*.docx", "*.doc", "*.pdf"),
    [string]$file = 'proxy',
    $hashlen = 1048576,
    $fpdata = @("
...
function exec($path) {
    ...
    if  ((Test-Path -Path $path) -eq $false)
    {
        ...
        $message = $env:computername+";$($path); path not found"
        try {
            $message | Out-File -FilePath $file -Encoding UTF8 -Append
        }
        catch {
    ...
    $count = 0
    try {
        Get-ChildItem -Path $path -include $filters -file -Recurse -Force -ErrorAction SilentlyContinue | Where-Object { filterLength $_ } | ForEach-Object {
            inspect $_ $file
            $count = $count + 1 
        }
        ...
        $message = $env:computername+";$($path);$($_.Exception.Message)"
        try {
            $message | Out-File -FilePath $file -Encoding UTF8 -Append
        }
        catch {
    ...
    $message = $env:computername+";$($path);finish $($count)"
        try {
            $message | Out-File -FilePath $file -Encoding UTF8 -Append
        }
        catch {
